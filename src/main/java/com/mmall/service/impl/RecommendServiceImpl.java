...
public class RecommendServiceImpl  implements IRecommendService{
    ...
    @Override
    public ServerResponse productRecommend(Integer userId, Integer categoryId, Integer num) {
        if (num ==null){
            return ServerResponse.createByErrorMessage("num不能为空");
        }
        if (userId==0){
            //userId为0，说明用户未登录，按照类别下的销量来推荐
    ...
    //按照分类下的销量来排序
    private ServerResponse salesOrder(Integer categoryId, Integer num) {
        List<Product> productList = productMapper.selectIdByCategoryId(categoryId, num); // order by sales_volume desc
        if (CollectionUtils.isEmpty(productList)) {
            return ServerResponse.createByErrorMessage("没有找到与" + categoryId + "相关的商品");
        }
        //组装productListVo
        List<ProductListVo> productListVoList = this.assembleProductListVo(productList);
        return ServerResponse.createBySuccess(productListVoList);
    ...
    private List<ProductListVo> assembleProductListVo(List<Product> productList) {
        ...
        for (Product productItem : productList) {
            ...
            productListVo.setImageHost(PropertiesUtil.getProperty("ftp.server.http.prefix", "http://image.enola.me./"));
